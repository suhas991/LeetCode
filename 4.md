...
* **Prop drilling** is the process of passing data from a **parent component** down to deeply nested **child components** through **props**, even if the intermediate components don’t need the data.
...
* React follows a **unidirectional data flow** (top → down).
...
* But **Parent** and **Child** don’t actually need it — they just forward it.
...
1. **Unnecessary code clutter** – Intermediate components pass props they don’t use.
2. **Harder maintenance** – If the prop structure changes, many components must be updated.
3. **Reduced readability** – Too many props make the component tree messy.
4. **Tight coupling** – Intermediate components depend on props unnecessarily.
...
👉 Now no intermediate component (`Parent`, `Child`) has to pass the `user` prop.
...
* In **small apps** or when data only needs to pass **1–2 levels deep**, prop drilling is fine.
...
* Use with `createContext` + `Provider` + `useContext` helps avoid prop drilling..
